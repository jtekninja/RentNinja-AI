...
function describeAiReviewFailure(error: unknown) {
  const rawMessage = error instanceof Error ? error.message : "Unable to generate AI analysis.";

  if (rawMessage.includes("followUpQuestions")) {
    return {
      logMessage: "AI review failed because the model returned too many follow-up questions for the required schema.",
      clientMessage: "AI review returned too many follow-up items. Please try again."
    };
  }

  if (rawMessage.includes("fetch failed") || rawMessage.includes("Unable to reach OpenAI")) {
    return {
      logMessage: "AI review failed because the app could not reach OpenAI. This is usually a network, timeout, or upstream availability issue.",
      clientMessage: "AI review could not reach OpenAI right now. Please try again in a moment."
    };
  }
  ...
  if (rawMessage.includes("OpenAI request failed")) {
    return {
      ...
      clientMessage: "AI review request was rejected by OpenAI. Please try again."
    };
  }

  return {
    logMessage: sanitizeAiText(rawMessage, 500) || "Unable to generate AI analysis.",
    clientMessage: sanitizeAiText(rawMessage, 220) || "Unable to generate AI analysis."
  };
}
  ...
  try {
    const analysis = await createStructuredOpenAIResponse({
      ...
      schema: {
        ...
        required: ["summary", "recommendation", "confidence", "strengths", "concerns", "followUpQuestions"],
        properties: {
          summary: { type: "string" },
          ...
          followUpQuestions: { type: "array", items: { type: "string" }, minItems: 1, maxItems: 5 }
        }
      },
      input: [
        {
        ...
        },
        {
          ...
          content: JSON.stringify({
            applicant: {
              ...
              responsibleRent: applicant.responsibleRent ?? applicant.monthlyRent,
              affordabilityRatio: applicant.affordabilityRatio,
              residentScore: applicant.residentScore ?? applicant.scores?.resident ?? 0,
    ...
    const normalizedAnalysis = {
      ...
      followUpQuestions: normalizeAnalysisArray(analysisRecord.followUpQuestions)
    };
    ...
    await recordAuditLog({
      ...
      metadata: {
        rawError: error instanceof Error ? sanitizeAiText(error.message, 800) : "Unknown error"
      }
    });
