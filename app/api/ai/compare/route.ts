...
export async function POST() {
  ...
  if (applicants.length < 2) {
    return NextResponse.json({ message: "Add at least two applicants to compare." }, { status: 400 });
  }

  try {
    const comparison = await createStructuredOpenAIResponse({
      schemaName: "applicant_comparison",
      schemaDescription: "A ranked comparison of tenant applicants for a landlord.",
      schema: {
        type: "object",
        additionalProperties: false,
        required: ["bestApplicantId", "ranking", "overview", "watchouts", "nextStep"],
        properties: {
    ...
    return NextResponse.json(
      { message: error instanceof Error ? error.message : "Unable to compare applicants." },
      { status: 500 }
    );
  }
}
