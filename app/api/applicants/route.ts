...
export async function GET() {
  ...
  return NextResponse.json(applicants.map((applicant) => serializeApplicantRecord(applicant)));
}
...
export async function POST(request: Request) {
  ...
  if (duplicateApplicant) {
    const duplicateMessage = "This applicant already exists in your workspace. Open the existing record and update it instead of creating a duplicate.";
    await recordAuditLog({
      organizationId: session.user.organizationId,
      actorUserId: session.user.id,
      actorName: session.user.name,
      actorEmail: session.user.email,
      action: "applicant.duplicate_blocked",
      entityType: "applicant",
      entityId: String(duplicateApplicant._id),
      level: "warning",
      message: `Blocked duplicate applicant create for ${parsed.data.email || parsed.data.name}.`,
      metadata: {
  ...
  try {
    ...
    await recordAuditLog({
      ...
      message: `Created applicant ${applicant.name}.`,
      metadata: {
    ...
    return NextResponse.json(serializeApplicantRecord(applicant.toObject()), { status: 201 });
  } catch (error) {
    if (error && typeof error === "object" && "code" in error && error.code === 11000) {
      await recordAuditLog({
        ...
        actorName: session.user.name,
        actorEmail: session.user.email,
        action: "applicant.duplicate_conflict",
        entityType: "applicant",
        level: "warning",
        message: `Duplicate applicant conflict for ${parsed.data.email || parsed.data.name}.`
      });
      return NextResponse.json(
        { message: "This applicant already exists in your workspace. Open the existing record and update it instead of creating a duplicate." },
        { status: 409 }
      );
    }
