...
export function ApplicantList({ applicants, onEdit, onDelete }: ApplicantListProps) {
  const [analysisById, setAnalysisById] = useState<Record<string, ApplicantAiAnalysis>>({});
  ...
  async function runAiReview(id: string) {
    ...
    try {
      ...
      if (!response.ok) {
        setErrorById((current) => ({ ...current, [id]: data.message || "Unable to generate AI review." }));
        return;
      }
      ...
      {applicants.map((applicant) => (
        <article key={applicant._id} className="rounded-[30px] border border-white/10 bg-white/5 p-5">
          {(() => {
            ...
            const normalizedScreeningScore =
              !isUnsetNumber(applicant.scores.resident)
            ...
            const rawResidentLabel =
              sourceLabel === "Apartments.com" && applicant.residentScore > 100 ? "ResidentScore" : "Source score";
            const responsibleRentLabel =
              applicant.housingSupport === "None" ? "Rent responsibility" : "Tenant share";
          ...
          <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
            <div>
              <div className="flex flex-wrap items-center gap-2">
                ...
                {applicant.coApplicants.length > 0 ? <StatusPill tone="neutral">Joint application</StatusPill> : null}
              </div>
              <p className="mt-2 text-sm text-slate-300">
                {applicant.email} | {applicant.phone} | Created {formatDate(applicant.createdAt)}
              </p>
              <p className="mt-1 text-sm text-slate-400">Source: {sourceLabel}</p>
            ...
            <div className="flex items-center gap-2">
              <div className="rounded-full border border-white/10 bg-white/5 px-4 py-2 text-sm font-semibold text-white">
                {isUnsetNumber(applicant.totalScore) ? "Pending score" : `${applicant.totalScore}/100`}
              </div>
              <Button variant="secondary" onClick={() => runAiReview(applicant._id)} disabled={loadingId === applicant._id}>
                {loadingId === applicant._id ? "Analyzing..." : analysis ? "Refresh AI review" : "AI review"}
              </Button>
          ...
          {analysis ? (
            <div className="mt-4 overflow-hidden rounded-[28px] border border-cyan-300/12 bg-[linear-gradient(180deg,rgba(114,182,255,0.12),rgba(255,255,255,0.03))] shadow-[0_18px_50px_rgba(0,0,0,0.18)]">
              <div className="border-b border-white/8 px-5 py-4">
                <div className="flex flex-wrap items-center gap-2">
                  <p className="text-xs uppercase tracking-[0.24em] text-cyan-100">AI Review</p>
                  <div className="h-1.5 w-1.5 rounded-full bg-cyan-200/80" />
                  <p className="text-sm text-slate-200">Landlord recommendation snapshot</p>
                </div>
                <div className="mt-3 flex flex-wrap items-center gap-2">
                <StatusPill tone={analysis.recommendation === "Approve" ? "strong" : analysis.recommendation === "Review" ? "review" : "risk"}>
                  {analysis.recommendation}
                </StatusPill>
                  <StatusPill tone="neutral">{analysis.confidence} confidence</StatusPill>
                </div>
              </div>
              <div className="px-5 py-5">
                <div className="rounded-[22px] border border-white/8 bg-black/15 px-4 py-4">
                  <p className="text-xs uppercase tracking-[0.22em] text-slate-300">Decision summary</p>
                  <p className="mt-3 max-w-5xl text-sm leading-7 text-slate-100">{analysis.summary}</p>
                </div>
                <div className="mt-4 grid gap-4 xl:grid-cols-3">
                  <InsightList title="Strengths" items={analysis.strengths} tone="strong" />
                  <InsightList title="Concerns" items={analysis.concerns} tone="review" />
                  <InsightList title="Follow-up" items={analysis.followUpQuestions} tone="neutral" />
                </div>
              </div>
            </div>
          ) : null}
          ...
          <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
            <Metric label="Rent" value={isUnsetNumber(applicant.monthlyRent) ? null : formatCurrency(applicant.monthlyRent)} />
            <Metric label="Household income" value={isUnsetNumber(applicant.monthlyIncome) ? null : formatCurrency(applicant.monthlyIncome)} />
            <Metric label="Affordability" value={isUnsetNumber(applicant.responsibleRent) || isUnsetNumber(applicant.monthlyIncome) ? null : `${applicant.affordabilityRatio.toFixed(1)}x`} />
            <Metric
          ...
          <div className="mt-3 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
            <Metric
              ...
              value={isUnsetNumber(normalizedScreeningScore) ? null : `${normalizedScreeningScore}/100`}
            />
            <Metric label="Application source" value={sourceLabel} />
            <Metric label="Property address" value={applicant.propertyAddress || null} />
            <Metric label="Property jurisdiction" value={[applicant.propertyCity, applicant.propertyState, applicant.propertyPostalCode].filter(Boolean).join(", ") || null} />
            <Metric label="Move-in date" value={applicant.moveInDate || null} />
            <Metric label="Housing assistance" value={applicant.housingSupport === "None" ? "None" : applicant.supportProgram || applicant.housingSupport} />
            <Metric label={responsibleRentLabel} value={isUnsetNumber(applicant.responsibleRent) ? null : formatCurrency(applicant.responsibleRent)} />
          </div>

          <div className="mt-3 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
            <Metric label="Subsidy amount" value={isUnsetNumber(applicant.monthlySubsidyAmount) ? null : formatCurrency(applicant.monthlySubsidyAmount)} />
            <Metric label="Assistance verification" value={applicant.housingSupport === "None" ? "N/A" : applicant.subsidyStatus} />
            <Metric label="Inspection" value={applicant.housingSupport === "None" ? "N/A" : applicant.inspectionStatus} />
          </div>
          ...
          <div className="mt-5 grid gap-4 xl:grid-cols-[1.3fr,0.7fr]">
            <div className="rounded-[24px] border border-white/8 bg-black/15 p-4">
              ...
              <div className="mt-3 grid gap-2 sm:grid-cols-2">
                ...
                <Score label="Screening" value={normalizedScreeningScore} />
          ...
          {applicant.coApplicants.length > 0 ? (
            <div className="mt-4 rounded-[24px] border border-white/8 bg-black/15 p-4">
              <p className="text-xs uppercase tracking-[0.22em] text-slate-300">Household applicants</p>
              <div className="mt-3 grid gap-3 xl:grid-cols-2">
                ...
                {applicant.coApplicants.map((coApplicant) => (
                  <div key={`${coApplicant.name}-${coApplicant.email}-${coApplicant.phone}`} className="rounded-[22px] border border-white/8 bg-white/5 p-4">
                    <p className="text-xs uppercase tracking-[0.22em] text-slate-300">Co-applicant</p>
                    <p className="mt-2 text-lg font-semibold text-white">{coApplicant.name}</p>
                    <p className="mt-2 text-sm text-slate-300">
                      {[coApplicant.email, coApplicant.phone].filter(Boolean).join(" | ") || "Contact details not provided"}
                    </p>
                    <p className="mt-3 text-sm font-semibold text-white">
                      {isUnsetNumber(coApplicant.monthlyIncome) ? "Income not provided" : `Monthly income ${formatCurrency(coApplicant.monthlyIncome)}`}
                    </p>
                    <p className="mt-2 text-sm text-slate-300">
                      {isUnsetNumber(coApplicant.residentScore) ? "ResidentScore not provided" : `ResidentScore ${coApplicant.residentScore}`}
                    </p>
                    {coApplicant.notes ? <p className="mt-2 text-sm text-slate-300">{coApplicant.notes}</p> : null}
                  </div>
                ))}
              </div>
            </div>
...
function Metric({ label, value }: { label: string; value: string | null }) {
  return (
    <div className="rounded-[22px] border border-white/8 bg-black/15 p-4">
      ...
      <p className={`mt-2 text-lg font-semibold ${value ? "text-white" : "text-slate-500"}`}>{value ?? "Not provided"}</p>
    </div>
  );
}
...
function Score({ label, value }: { label: string; value: number }) {
  return (
    <div className="rounded-2xl border border-white/8 bg-white/5 px-3 py-2">
      ...
      <p className={`mt-1 text-base font-semibold ${isUnsetNumber(value) ? "text-slate-500" : "text-white"}`}>
  ...
  return (
    <div className="rounded-[22px] border border-white/8 bg-black/15 p-4">
      ...
      <ul className="mt-3 grid gap-2.5 text-sm text-slate-100">
        {items.map((item) => (
          <li key={item} className={`flex items-start gap-3 rounded-2xl border px-3 py-3 leading-6 ${chipClass}`}>
...
function NoteSection({ note }: { note: string }) {
  ...
  return (
    <li className="rounded-2xl border border-white/8 bg-white/5 px-4 py-3">
      <p className="text-xs font-semibold uppercase tracking-[0.22em] text-[#f7b36d]">{heading}</p>
      {hasBody ? (
